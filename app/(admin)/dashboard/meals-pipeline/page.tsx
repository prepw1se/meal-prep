...
import { Clock, MapPin, Phone, ChefHat, Truck, CheckCircle, AlertCircle, Timer, Search, Filter, Plus } from "lucide-react"
...
export default function PipelinePage() {
  ...
  const moveOrder = (orderId: string, newStage: string) => {
    setOrders((prev) => prev.map((order) => (order.id === orderId ? { ...order, stage: newStage } : order)))
  }
    ...
    <Card className="mb-3 cursor-pointer hover:shadow-md transition-shadow border-gray-200">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium text-gray-900">{order.id}</CardTitle>
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center space-x-2">
          <Avatar className="h-8 w-8 bg-gray-100">
            <AvatarFallback className="text-gray-700">{order.customer.initials}</AvatarFallback>
          </Avatar>
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="px-4 md:px-6 py-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">Meals Pipeline</h1>
